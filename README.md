...
├── finetune/
│   ├── data/
│   │   ├── commands.jsonl        # 271 NL→command training pairs
│   │   └── train.jsonl           # Generated: ChatML format
│   ├── prepare_data.py           # Validate + convert commands.jsonl → train.jsonl
│   ├── finetune.py               # LoRA/QLoRA training with SFTTrainer
│   ├── export_model.py           # Merge LoRA into base model → merged-model/
│   ├── inference_server.py       # Optional: FastAPI server (alternative runtime)
│   └── requirements.txt
│
...
makes transformers and PEFT. It has a native Qwen2 implementation that loads
the same `model.safetensors` + `config.json` files that Python produces.

At runtime:
1. `neuroshell` memory-maps `model.safetensors` (fast startup, OS handles caching)
2. Formats the task into a ChatML prompt
3. Runs a prefill pass (processes the whole prompt at once)
4. Greedily decodes one token at a time until `<|im_end|>` or EOS
5. Takes the first line of output as the command
6. Clears the KV cache for the next call

**Memory:** ~1 GB RAM (float16 weights for 0.5B model, memory-mapped)
